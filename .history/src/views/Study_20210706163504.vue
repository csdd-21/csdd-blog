<template>
  <div class="study">
    <div class="contentWrapper col-xs-12">
      <FilterCond class="filter col-xs-12 col-md-8" :tags="tags" @handleFilter="handleFilter"></FilterCond>
      <div :style="study.datas.length == 0 ? empty.show : empty.hide"><i class="iconfont icon-empty"></i>{{ empty.info }}</div>
      ...
      <div class="page col-xs-12 col-md-8" :key="page.skip" :style="study.datas.length == 0 ? empty.hide : ''">
...
export default {
  ...
  data() {
    return {
      routerKey: "",
      empty: {
        info: "Opps, there is no data for Study page ~",
        show: {
          display: "block",
          "text-align": "center",
          "margin-top": "42px",
  ...
  methods: {
    ...
    handleFilter(i) {
      this.SET_PARAMS({ parent: "study", title: i.title, checkTags: i.tags, sort: i.sort });
  ...
  created() {},
  mounted() {
    this.getTags("study").then(() => { 
      this.SET_PARAMS({ parent: "study"});
      this.getArticles("study");
    });
    this.SET_SHOW_FOOTER({ showFooter: true });
  },
  unmounted() {
    this.SET_PARAMS_DEFAULT({ parent: "study"});
  },
};
</script>
